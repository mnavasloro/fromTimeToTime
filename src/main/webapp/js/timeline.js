...
Date.prototype.getVerboseDate = function(includeDayName = false) {
  let dayName = "";
  if (includeDayName) {
    includeDayName += this.DAYSOTW[this.getDay()];
  }
...
function printSVGLines(lEventArray, svgInDOM, doSort = true, randomizeTest = false, overwrite = true) {
  ...
  if (logsOn) {
    console.groupCollapsed("%c[MA] %c Transforming temporal data into Date()", "color: #0e4a75; font-weight:bold;", "text-decoration:underline;")
  };
  ...
  for (event in lEventArray) {
    ...
    if (typeof(lEventArray[event].timeStamp) != "undefined") {
      if (logsOn) {
        console.log(`\t Event ${event} has timeStamp already defined: ${lEventArray[event].timeStamp}`);
        continue;
      }
    }
    ...
    for (attr in lEventArray[event]["ATTRIBUTES"]) {
      ...
      if (lEventArray[event]["ATTRIBUTES"][attr].INFO === "when") {
        ...
        if (logsOn) {
          console.log(`\t\t new Date() involving ${event}; value:${lEventArray[event].timeStamp}`);
        };
        countTemporal++;
      }
    }
  ...
  // MARIA: AQUI FALTA TRANSFORMAR EL JSON EN HTML (y ponerlo en svgInDOM)
  ...
  */
  ...
  /*	for(let iEv= 0; iEv < lEventArray.length; iEv++){
  		...
  		console.log(`%c Creando outermost node ; Event: ${iEv}; Relevance${lEventArray[iEv].RELEVANCE}; radiusMainNode: ${radiusMainNode_BasedOnRelevance}`, "color:red;");
  			...
  			if( lEventArray[iEv].hasTime >0 && typeof lEventArray[iEv].timeStamp !== undefined ){
  				...
  				createEventNodeGroup(lEventArray[iEv]._evID,marginTime_line+iEv*eventTimelineConstantSeparation,svgDim.height/2,lEventArray[iEv],radiusMainNode_BasedOnRelevance,svgInDOM,markerDirAlternating);
  				...
  				createEventNodeGroup(lEventArray[iEv]._evID,  marginNon_timeline + non_timed_currentSeparation + (2/3)*svgDim.width,  (svgDim.height - (2/3)*svgDim.height)/2 +(2/3)*svgDim.height ,lEventArray[iEv],radiusMainNode_BasedOnRelevance,svgInDOM, markerDirAlternating);
  ...
  function createMarker(lEvent, direction = true, heightMarker, radiusMarker) {
    /* CREATE MARKERS */
    let _markerNewG = createSVGTag("g", {
      id: `marker_${lEvent._evID}`
    }); //Crea un grupo que va a contener los componentes de un marker que corresponde a cada evento, mostrando el tipo de evento que es.
    ...
    let _markerInnerCircle = createSVGTag("circle", {
      cx: lEvent.cMx,
      cy: lEvent.cMy,
      r: radiusMarker,
      class: "innerMarkerCircle"
    });

    let _markerOuterCircle = createSVGTag("circle", {
      cx: lEvent.cMx,
      cy: lEvent.cMy,
      r: radiusMarker + (15 / 100 * radiusMarker),
      class: lEvent.getMainNodeStylingClass()
    });

    //We obtain the coordinates and lenght of the square side that allows to fit an image perfectly centered in our marker.
    let fitCircleCoords = {
      fcx: lEvent.cMx - (Math.sqrt(Math.pow(radiusMarker, 2) / 2)),
      fcy: lEvent.cMy - (Math.sqrt(Math.pow(radiusMarker, 2) / 2)),
      l: (Math.sqrt(Math.pow(radiusMarker, 2) / 2)) * 2
    }

    let _markerIcon = createSVGTag("image", {
      x: fitCircleCoords.fcx,
      y: fitCircleCoords.fcy,
      height: fitCircleCoords.l,
      width: fitCircleCoords.l,
      href: `${lEvent.getAssociatedEventImgPath()}`
    });

    //POINTER
    let pathConnect = createSVGTag("path", {
      d: `M ${fitCircleCoords.fcx+8} ${fitCircleCoords.fcy+fitCircleCoords.l} L ${lEvent.cnx} ${lEvent.cny}  L ${fitCircleCoords.fcx+fitCircleCoords.l-8} ${fitCircleCoords.fcy+fitCircleCoords.l} Z`,
      class: "innerMarkerCircle"
    });

    //Anchor element que ha de conducir al evento dento del texto como tal [@REVISIT]
    let anchorElem = createSVGTag("a", {
      href: `#${lEvent.inTextAnnotationNode.id}`
    });
    ...
    if (auxGuidelines) {
      ...
      function createPointAnnotation(label, _cx, _cy, textAt = "middle", color, appendTo, _dx = 0, _dy = 0) {
        ...
        let auxCircle = createSVGTag("circle", {
          cx: _cx,
          cy: _cy,
          r: 2,
          fill: color
        });
        let textNameCoords = createSVGTag("text", {
          x: _cx,
          y: _cy,
          "text-anchor": textAt,
          fill: color,
      ...
      let auxLine = createSVGTag("path", {
        d: `M ${fitCircleCoords.fcx} ${fitCircleCoords.fcy} h ${fitCircleCoords.l}`,
        stroke: "blue"
      });

      let textNameCoords = createSVGTag("text", {
        x: (fitCircleCoords.fcx + fitCircleCoords.l) / 2,
        y: fitCircleCoords.fcx - 5,
        "text-anchor": "middle",
        fill: "blue",
        "font-size": 10
      });
  ...
  function buildGraph(idGraph, lEvent, outNodeDistance, outNodeRadius, appendTo, radiusMN = 15) {
    ...
    let eventCircle = createSVGTag("circle", {
      cx: lEvent.cnx,
      cy: lEvent.cny,
      r: radiusMN,
      class: lEvent.getMainNodeStylingClass()
    ...
    let textNodeMain = createSVGTag("text", {
      x: lEvent.cnx,
      y: lEvent.cny,
      class: "mainNodeText",
      style: `font-size:${(radiusMN/20*8)}px`
    });
    ...
    if (lEvent["ATTRIBUTES"] === undefined) {
      return appendTo;
    }
    let alpha = {
      deg: 360 / (lEvent["ATTRIBUTES"].length)
    };
    ...
    let groupBranches = createSVGTag("g", {
      id: `${lEvent._evID}` + "_Braches"
    });
    ...
    for (lAttr in lEvent["ATTRIBUTES"]) {
      ...
      let connectLine_i = createSVGTag("path", {
        d: `M ${lEvent.cnx} ${lEvent.cny} L ${lEvent["ATTRIBUTES"][lAttr].cAx} ${lEvent["ATTRIBUTES"][lAttr].cAy}`,
        class: "interNodeLine"
      });

      let outerNode_i = createSVGTag("circle", {
        cx: lEvent["ATTRIBUTES"][lAttr].cAx,
        cy: lEvent["ATTRIBUTES"][lAttr].cAy,
        r: outNodeRadius,
        class: "outerNodeGraph"
      });
      ...
      let textOuterNode = createSVGTag("text", {
        x: lEvent["ATTRIBUTES"][lAttr].cAx,
        y: lEvent["ATTRIBUTES"][lAttr].cAy,
        dy: -(outNodeRadius / 2),
        class: "outerNodeTextLabel",
        style: `font-size:${(outNodeRadius/10*6)}px`
      });
      ...
      while (startIndex < arrayWholeDesc.length) {
        ...
        if (processedCount > 1 || arrayWholeDesc.length < 1) { //Cada 3 palabras añadimos un fragmento al array de nodos de texto.
          let textOuterNodeDesc_i = createSVGTag("text", {
            x: lEvent["ATTRIBUTES"][lAttr].cAx,
            y: lEvent["ATTRIBUTES"][lAttr].cAy,
            dy: positionHandler * outNodeRadius / 10 * 6,
            class: "outerNodeTextDesc",
            style: `font-size:${(outNodeRadius/10*6)}px`
          });
